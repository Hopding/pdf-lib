...
describe('PDFRadioButton', () => {
    ...
    describe('can toggle an option', () => {
        ...
        beforeEach(() => {
            onAppearanceStream1 = new PDFStream(PDFDict.fromMapWithContext(new Map(), context));
            onAppearanceStream1Ref = context.register(onAppearanceStream1);
            offAppearanceStream = new PDFStream(PDFDict.fromMapWithContext(new Map(), context));
            offAppearanceStreamRef = context.register(offAppearanceStream);
            onAppearanceStream2 =  new PDFStream(PDFDict.fromMapWithContext(new Map(), context));
            ...
            firstChildAppearanceDict = PDFDict.fromMapWithContext(
                new Map([
                    [PDFName.N, firstChildNormalAppearanceDict]
                ]),
                context
            );
            secondChildAppearanceDict = PDFDict.fromMapWithContext(
                new Map([
                    [PDFName.N, secondChildNormalAppearanceDict]
                ]),
                context
            );
