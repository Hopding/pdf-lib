...
async function buildPdf() {
  ...
  const assets = {
    ubuntuFontBytes: await getAsset('./assets/ubuntu-fonts/Ubuntu-R.ttf'),
    unicornJpgBytes: await getAsset('./assets/cat_riding_unicorn.jpg'),
    marioPngBytes: await getAsset('./assets/running_mario.png'),
  };
  ...
  const [unicornJpgRef, unicornJpgDims] = pdfDoc.embedJPG(
    assets.unicornJpgBytes,
  );
  ...
  const contentStream1 = pdfDoc.createContentStream(
    ...
    drawText(
      helveticaFont.encodeText('This PDF was Created with JavaScript!'),
      {
        x: 85,
        y: PAGE_1_HEIGHT - 48,
        font: HELVETICA_FONT,
