...
const drawLinesContentStream = (
  ...
  pdfDoc.register(
    pdfDoc.createContentStream(
      drawText(font.encodeText(fontName), {
        font: fontName,
        size,
        x: 650 / 2 - font.widthOfTextAtSize(fontName, size) / 2,
        y: yPosition,
      }),
      drawLinesOfText(lines.map(font.encodeText), {
        font: fontName,
        size,
        x: 25,
        y: yPosition - font.heightOfFontAtSize(size) - 10,
        lineHeight: font.heightOfFontAtSize(size) + 10,
      }),
    ),
  );
...
const addPageWithFonts = (
  ...
  const page = pdfDoc.createPage([650, 700]);
  let yPos = 675;

  fontNames.forEach((fontName) => {
    const [fontRef, font] = pdfDoc.embedStandardFont(fontName);
    const lines = breakTextIntoLines(text, fontSize, font, 600);
    const csRef = drawLinesContentStream(
      pdfDoc,
      lines,
      font,
      fontName,
    ...
    yPos -= (font.heightOfFontAtSize(fontSize) + 10) * (lines.length + gapAmt);
    page.addFontDictionary(fontName, fontRef).addContentStreams(csRef);
  });
...
const kernel = (assets: ITestAssets) => {
  ...
  const zapfDingbatsContentStreamRef = pdfDoc.register(
    pdfDoc.createContentStream(
      // prettier-ignore
      drawText(helveticaFont.encodeText('ZapfDingbats'), {
        font: 'Helvetica',
      ...
      drawLinesOfText(zapfDingbatsLines.map(zapfDingbatsFont.encodeText), {
        font: 'ZapfDingbats',
        size: zapfDingbatsFontSize,
        x: 25,
        y: yPos -=
          zapfDingbatsFont.heightOfFontAtSize(zapfDingbatsFontSize) + 10,
        lineHeight:
          zapfDingbatsFont.heightOfFontAtSize(zapfDingbatsFontSize) + 10,
      }),
    ),
  ...
  const symbolContentStreamRef = pdfDoc.register(
    pdfDoc.createContentStream(
      // prettier-ignore
      drawText(helveticaFont.encodeText('Symbol'), {
        font: 'Helvetica',
        size: symbolFontSize,
        x: (650 / 2) -
          (helveticaFont.widthOfTextAtSize('Symbol', symbolFontSize) / 2),
        y: yPos -= 275,
      }),
      drawLinesOfText(symbolLines.map(symbolFont.encodeText), {
        font: 'Symbol',
        size: symbolFontSize,
        x: 25,
        y: yPos -= symbolFont.heightOfFontAtSize(symbolFontSize) + 10,
        lineHeight: symbolFont.heightOfFontAtSize(symbolFontSize) + 10,
      }),
    ),
  );
  ...
  const page = pdfDoc
    .createPage([650, 700])
    .addFontDictionary('ZapfDingbats', zapfDingbatsRef)
